...
func ReadBlock(db ethdb.Reader, hash common.Hash, number uint64) *types.Block {
	...
	return types.NewBlockWithHeader(header).WithBody(body.Transactions, body.Uncles, body.Msgs)
}
...
func ReadBadBlock(db ethdb.Reader, hash common.Hash) *types.Block {
	...
	for _, bad := range badBlocks {
		if bad.Header.Hash() == hash {
			return types.NewBlockWithHeader(bad.Header).WithBody(bad.Body.Transactions, bad.Body.Uncles, bad.Body.Msgs)
		}
	}
	return nil
}
...
func ReadAllBadBlocks(db ethdb.Reader) []*types.Block {
	...
	for _, bad := range badBlocks {
		blocks = append(blocks, types.NewBlockWithHeader(bad.Header).WithBody(bad.Body.Transactions, bad.Body.Uncles, bad.Body.Msgs))
	}
	return blocks
}
