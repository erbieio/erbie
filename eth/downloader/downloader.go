...
func (d *Downloader) fetchBodies(from uint64) error {
	...
	var (
		deliver = func(packet dataPack) (int, error) {
			...
			return d.queue.DeliverBodies(pack.peerID, pack.transactions, pack.uncles, pack.msgs)
		}
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	for i, result := range results {
		blocks[i] = types.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles, result.Msgs)
	}
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	block := types.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles, result.Msgs)
...
func (d *Downloader) DeliverBodies(id string, transactions [][]*types.Transaction, uncles [][]*types.Header, msgs [][]*web2msg.ProtocolMsg) error {
	return d.deliver(d.bodyCh, &bodyPack{id, transactions, uncles, msgs}, bodyInMeter, bodyDropMeter)
}
