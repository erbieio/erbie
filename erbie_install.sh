#!/bin/bash
#check docker cmd
echo "Script version Number: v1.0.2"
...
erb=$(docker images|grep "erbie/erbie"|grep "latest")

if [ -n "$erb" ];then
#erbie/erbie:latest image in machine
        ct=$(docker inspect erbie/erbie:latest -f {{.Created}})
        cts=$(date -d "$ct" +%s)
        if [ $cts -eq $vl ];then
        #version is latest
                container=$(docker ps -a|awk '{if($NF == "erbie")print $0}')
                if [[ $container =~ "Up" ]];then
                #erbie container is running
                        while true
                        do
                                key=$(docker exec -it erbie /usr/bin/ls -l /erb/.erbie/erbie/nodekey)
                                if [ -n "$key" ];then
                                #nodekey is alive
                                        echo -e "Erbie is the latest versiob: $vr \nnode private key:"
                ...
                docker rmi erbie/erbie:latest > /dev/null 2>&1
                if [ $cts -lt $vt5 ];then
...
docker pull erbie/erbie:latest

docker images|awk '{print $1,$2}'|while read img ver
do
        if [[ $img == "erbie/erbie" ]] && [[ $ver == "latest" ]];then
                echo "true">temp
                break
        fi
done
...
docker run -id -p 30303:30303 -p 8545:8545 -v /erb/.erbie:/erb/.erbie --name erbie erbie/erbie:latest
...
do
	...
	#echo "ls -l /erb/.erbie/erbie/nodekey:"$key
        if [[ $s -gt 0 ]] && [[ "$key" =~ "root" ]];then
                echo "node private key:"
                docker exec -it erbie /usr/bin/cat /erb/.erbie/erbie/nodekey
                echo -ne "\n"
